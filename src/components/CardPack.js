...
const CardPack = ({ onAddToAlbum, onBurst, album = [] }) => {
    ...
    return (
      <div className="mb-10">
        ...
        {openedCards.map((card, index) => {
            const isDuplicate = album.some((c) => c.id === card.id);
        return (
            <Card key={index + card.name} card={card} onBurst={onBurst} duplicate={isDuplicate}
            />
        );
    })}
        </div>
